...
for i = 1:50
    for j = 1:50
        H(j,i) = Tr(i,3)*Tr(j,3)*((Tr(j,1:2)*Tr(i,1:2)')^P);
    end
end
...
Kb = 0;
for i = 1:50
    Kb = Kb+(alpha(i)*Tr(i,3)*((Tr(i,1:2)*Tr(ak(1),1:2)')^P));
end
...
for i = 1:50
    for j = 1:50
        Dt(i) = Dt(i)+(alpha(j)*Tr(j,3)*((Tr(j,1:2)*Te(i,1:2)')^P));
    end
end
Dt = Dt+bias;
for i = 1:50
...
T1 = table(t1(:,1),t1(:,2),t1(:,3),t1(:,4),t1(:,5), ...
    'VariableNames',{'poly=1','poly=2','poly=3','poly=4','poly=5'}, ...
